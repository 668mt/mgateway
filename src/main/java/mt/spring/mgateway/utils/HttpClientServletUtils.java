...
public class HttpClientServletUtils {
	...
	public static void forward(CloseableHttpClient httpclient, String url, HttpServletRequest request, HttpServletResponse response, OutputStream outputStream, @Nullable Map<String, String> responseHeaders, @Nullable ResponseHandler responseHandler) throws Exception {
		forward(httpclient, url, request, response, outputStream, null, null, responseHandler);
	}
	
	public static void forward(CloseableHttpClient httpclient, String url, HttpServletRequest request, HttpServletResponse response, OutputStream outputStream, @Nullable Map<String, String> requestHeaders, @Nullable Map<String, String> responseHeaders, @Nullable ResponseHandler responseHandler) throws Exception {
//		uriFactory.setEncodingMode(DefaultUriBuilderFactory.EncodingMode.URI_COMPONENT);
//		URI uri = uriFactory.expand(url);
		URI uri = URI.create(url);
		forward(httpclient, uri, request, response, outputStream, requestHeaders, responseHeaders, responseHandler);
	}
	
	public static void forward(CloseableHttpClient httpclient, URI uri, HttpServletRequest request, HttpServletResponse response, OutputStream outputStream, @Nullable Map<String, String> requestHeaders, @Nullable Map<String, String> responseHeaders, @Nullable ResponseHandler responseHandler) throws Exception {
		...
		HttpRequest httpRequest = buildHttpRequest(request.getMethod().toUpperCase(), uri.toString(), entity, headers, queryParams);
		...
		writeResponse(closeableHttpResponse, request, response, outputStream, responseHeaders, responseHandler);
	}
...
//				,"Transfer-Encoding"
	...
	public static void writeResponse(CloseableHttpResponse closeableHttpResponse, HttpServletRequest request, HttpServletResponse response, OutputStream outputStream, @Nullable Map<String, String> responseHeaders, @Nullable ResponseHandler responseHandler) throws Exception {
		...
		if (responseHandler != null) {
			responseHandler.beforeWriteContent(closeableHttpResponse, response);
		}
	...
	public static HttpRequest buildHttpRequest(String verb, String uri, HttpEntity entity, MultiValueMap<String, String> headers, MultiValueMap<String, String> queryParams) {
		if (queryParams.size() > 0) {
			uri += "?" + buildQueryParam(queryParams);
		}
		try {
			return buildHttpRequest(verb, new URL(uri).toURI(), entity, headers);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	
	public static HttpRequest buildHttpRequest(String verb, URI uri, HttpEntity entity, MultiValueMap<String, String> headers) {
